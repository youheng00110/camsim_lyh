...
cp -f "$RAFT_SOURCE" "$RAFT_CACHE"
cp -f "$LOFTR_SOURCE" "$LOFTR_CACHE"
...
echo
echo "============================================================"
echo "Verify RAFT and LoFTR checkpoints"
echo "============================================================"
...
for item_index in [0, len(items) - 1]:
    ...
    frame_indices = [0, len(item["frames"]) - 1]

    for frame_index in frame_indices:
        frame = item["frames"][frame_index]

        if not frame.get("views"):
            print(
        ...
        image_path = frame["views"][0].get("image_path")
...
PY
    ...
    echo "Merged result is missing or invalid."
    echo "Rebuild: $DST"
    ...
    if [ ! -f "$MANIFEST" ]
    then
        echo "Manifest not found after merge: $MANIFEST"
        exit 1
    fi


    VIDEO_COUNT=$(python - "$MANIFEST" <<'PY'
import sys
...
PY
        ...
        if ! python -m dwm.tools.evaluate_fvd_from_paired_manifest \
            ...
            2>&1 | tee "$FVD_LOG"
        then
            echo "[GPU $GPU] batch-size=2 failed."
...
#
# 汇总写入：
#   nuplanhard1000/fvd_detection_summary.json
# ============================================================
...
print()
print(
    "| Index | Method | FVD status | FVD value | Result file |"
)
print(
    "|---:|---|---|---:|---|"
)
...
for index, name in enumerate(names):
    ...
    if isinstance(fvd_value, (int, float)):
        valid_value = math.isfinite(float(fvd_value))
